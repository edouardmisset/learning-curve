...
Modern React is heavily functional (Hooks, Functional Components). On the other hand, Ember has embraced modern JavaScript **classes** and **[Decorators](/learning-curve/blog/15-oop#decorators)**. Decorators (like `@tracked`, `@action`, `@service`) are just functions that wrap classes or properties to add behavior. This is somewhat similar to Higher-Order Components in older React.

> See the article on [Object-Oriented Programming](/learning-curve/blog/15-oop) for a refresher on classes and inheritance, which are fundamental to Ember's architecture.
...
> See [Memoization](/learning-curve/blog/04-memoization) for how React handles derived state.
